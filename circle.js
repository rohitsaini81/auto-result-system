...
const DL_bazar = async () => {
  ...
  let obj = {}
  const out = await result.Bfunc().then((res) => {
...
// ---------------------------->
const Faridabad = async() => {
  if (faridabadCalled) {
    return false;
  }
  console.log("Faridabad");
  const out = await result.Afunc().then((res) => {
...
const Gali = async() => {
  ...
  //   console.log(res);
  // })
 const out =  result.Afunc().then((res) => {
